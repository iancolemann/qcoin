...
static const char USAGE[] = R"(qcoind )" qcoin_VERSION_STRING R"(.
...
Options:
  ...
  --qcoind-bind-address=<ip:port>     IP and port for qcoind RPC API [default: 127.0.0.1:8081].
  ...
  --data-folder=<folder-path>            Folder for blockchain, logs and peer DB [default: )" platform_DEFAULT_DATA_FOLDER_PATH_PREFIX
                            R"(qcoin].
  --qcoind-authorization=<usr:pass>   HTTP basic authentication credentials for RPC API.
  --qcoind-authorization-private=<usr:pass>   HTTP basic authentication credentials for get_statistics and get_archive methods.)";
...
int main(int argc, const char *argv[]) try {
	...
	if (!export_blocks.empty() && !backup_blockchain.empty()) {
		std::cout << "You can either export blocks or backup blockchain on one run of qcoind" << std::endl;
		return api::BYTECOIND_WRONG_ARGS;
	}
	...
	std::cout << "qcoind started seconds=" << double(idea_ms.count()) / 1000 << std::endl;
	while (!io.stopped()) {
