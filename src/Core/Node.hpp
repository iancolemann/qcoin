...
protected:
	...
	class P2PProtocolqCoin;
	struct DownloadInfo {
		size_t chain_counter                 = 0;
		P2PProtocolqCoin *who_downloading = nullptr;
	...
	std::map<Hash, P2PProtocolqCoin *> downloading_transactions;

	class P2PProtocolqCoin : public P2PProtocolBasic {
		Node *const m_node;
...
#endif
	public:
		explicit P2PProtocolqCoin(Node *node, P2PClient *client);
		~P2PProtocolqCoin() override;
	...
	std::unique_ptr<P2PProtocol> client_factory(P2PClient *client) {
		return std::make_unique<P2PProtocolqCoin>(this, client);
	}
	...
	std::set<P2PProtocolqCoin *> m_broadcast_protocols;

	BlockPreparatorMulticore m_pow_checker;
	// TODO - periodically clear m_pow_checker of blocks that were not asked

	void broadcast(P2PProtocolqCoin *exclude, const BinaryArray &data);
	void broadcast(P2PProtocolqCoin *exclude, const BinaryArray &data_v1, const BinaryArray &data_v4);
