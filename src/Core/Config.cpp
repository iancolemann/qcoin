...
Config::Config(common::CommandLine &cmd)
    ...
    , qcoind_bind_port(RPC_DEFAULT_PORT)
    , qcoind_bind_ip("127.0.0.1")  // Less attack vectors from outside for ordinary uses
    , qcoind_remote_ip("127.0.0.1")
	...
	if (const char *pa = cmd.get("--" CRYPTONOTE_NAME "d-authorization")) {
		qcoind_authorization         = common::base64::encode(BinaryArray(pa, pa + strlen(pa)));
		qcoind_authorization_private = qcoind_authorization;
	}
	if (const char *pa = cmd.get("--" CRYPTONOTE_NAME "d-authorization-private")) {
		qcoind_authorization_private = common::base64::encode(BinaryArray(pa, pa + strlen(pa)));
	}
	if (const char *pa = cmd.get("--" CRYPTONOTE_NAME "d-bind-address")) {
		ewrap(common::parse_ip_address_and_port(pa, &qcoind_bind_ip, &qcoind_bind_port),
	...
	if (const char *pa = cmd.get("--" CRYPTONOTE_NAME "d-remote-address")) {
		...
		if (addr.find(prefix) == 0) {
			...
			qcoind_remote_port = common::integer_cast<uint16_t>(sport);
			qcoind_remote_ip   = prefix + sip;
		} else {
			const std::string prefix2 = "http://";
			if (addr.find(prefix2) == 0)
				addr = addr.substr(prefix2.size());
			ewrap(common::parse_ip_address_and_port(addr, &qcoind_remote_ip, &qcoind_remote_port),
			    ConfigError(emsg));
		}
	}
...
std::string Config::prepare_usage(const std::string &usage) {
	...
	boost::replace_all(result, "qcoin", CRYPTONOTE_NAME);
