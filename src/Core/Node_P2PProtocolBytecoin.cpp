...
Node::P2PProtocolqCoin::P2PProtocolqCoin(Node *node, P2PClient *client)
    : P2PProtocolBasic(node->m_config, node->m_p2p.get_unique_number(), client)
    , m_node(node)
    , m_chain_timer(std::bind(&P2PProtocolqCoin::on_chain_timer, this))
    , m_download_timer(std::bind(&P2PProtocolqCoin::on_download_timer, this))
    , m_syncpool_timer(std::bind(&P2PProtocolqCoin::on_syncpool_timer, this))
    , m_download_transactions_timer(std::bind(&P2PProtocolqCoin::on_download_transactions_timer, this)) {}

Node::P2PProtocolqCoin::~P2PProtocolqCoin() {
	//	std::cout << "~P2PProtocolqCoin this=" << std::hex << (size_t)this << std::dec << std::endl;
}

void Node::P2PProtocolqCoin::on_msg_bytes(size_t, size_t) {  // downloaded. uploaded
	//    node->peers.on_peer_bytes(get_address(), downloaded, uploaded,
	//    node->p2p.get_local_time());
}

CoreSyncData Node::P2PProtocolqCoin::get_my_sync_data() const {
...
std::vector<PeerlistEntryLegacy> Node::P2PProtocolqCoin::get_peers_to_share(bool lots) const {
...
void Node::P2PProtocolqCoin::on_first_message_after_handshake() {
...
void Node::P2PProtocolqCoin::on_chain_timer() {
...
void Node::P2PProtocolqCoin::on_download_transactions_timer() {
	invariant(m_downloading_transaction_count != 0, "");
	m_node->m_log(logging::TRACE) << "on_download_transactions_timer, disconnecting " << get_address() << std::endl;
	disconnect(std::string());
}

void Node::P2PProtocolqCoin::advance_chain() {
...
void Node::P2PProtocolqCoin::advance_blocks() {
...
void Node::P2PProtocolqCoin::advance_transactions() {
...
bool Node::P2PProtocolqCoin::on_transaction_descs(const std::vector<TransactionDesc> &descs) {
...
void Node::P2PProtocolqCoin::transaction_download_finished(const Hash &tid, bool success) {
...
bool Node::P2PProtocolqCoin::on_idle(std::chrono::steady_clock::time_point idle_start) {
...
void Node::P2PProtocolqCoin::after_handshake() {
...
void Node::P2PProtocolqCoin::on_msg_handshake(p2p::Handshake::Request &&req) {
	m_node->m_peer_db.add_incoming_peer(get_address(), m_node->m_p2p.get_local_time());
	after_handshake();
}

void Node::P2PProtocolqCoin::on_msg_handshake(p2p::Handshake::Response &&req) {
	m_node->m_peer_db.merge_peerlist_from_p2p(get_address(), req.local_peerlist, m_node->m_p2p.get_local_time());
	after_handshake();
}

void Node::P2PProtocolqCoin::on_msg_notify_request_chain(p2p::GetChainRequest::Notify &&req) {
...
void Node::P2PProtocolqCoin::on_msg_notify_request_chain(p2p::GetChainResponse::Notify &&req) {
	if (get_peer_version() >= P2PProtocolVersion::AMETHYST) {
...
void Node::P2PProtocolqCoin::on_msg_notify_request_objects(p2p::GetObjectsRequest::Notify &&req) {
...
void Node::P2PProtocolqCoin::on_msg_notify_request_objects(p2p::GetObjectsResponse::Notify &&req) {
	if (get_peer_version() >= P2PProtocolVersion::AMETHYST && req.blocks.size() > 1)
...
void Node::P2PProtocolqCoin::on_disconnect(const std::string &ban_reason) {
...
void Node::P2PProtocolqCoin::on_msg_notify_request_tx_pool(p2p::SyncPool::Notify &&req) {
	if (get_peer_version() >= P2PProtocolVersion::AMETHYST)
...
void Node::P2PProtocolqCoin::on_msg_notify_request_tx_pool(p2p::SyncPool::Request &&req) {
	if (req.from <= req.to)
...
void Node::P2PProtocolqCoin::on_msg_notify_request_tx_pool(p2p::SyncPool::Response &&req) {
	m_syncpool_timer.cancel();
	m_syncpool_equest_sent = false;
...
void Node::P2PProtocolqCoin::on_msg_timed_sync(p2p::TimedSync::Request &&req) { advance_chain(); }
void Node::P2PProtocolqCoin::on_msg_timed_sync(p2p::TimedSync::Response &&req) { advance_chain(); }

void Node::P2PProtocolqCoin::on_msg_notify_new_block(p2p::RelayBlock::Notify &&req) {
	if (get_peer_version() >= P2PProtocolVersion::AMETHYST) {
...
void Node::P2PProtocolqCoin::on_msg_notify_new_transactions(p2p::RelayTransactions::Notify &&req) {
	if (get_peer_version() >= P2PProtocolVersion::AMETHYST) {
		if (!req.txs.empty())
...
void Node::P2PProtocolqCoin::on_msg_notify_checkpoint(p2p::Checkpoint::Notify &&req) {
...
#if qcoin_ALLOW_DEBUG_COMMANDS

void Node::P2PProtocolqCoin::on_msg_stat_info(p2p::GetStatInfo::Request &&req) {
