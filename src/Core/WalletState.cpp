...
void WalletState::DeltaState::add_transaction(
	...
	// 1. when they are added from PQ, then come from qcoind
...
bool WalletState::sync_with_blockchain(api::cnd::SyncMemPool::Response &resp) {
	...
	if (resp.added_raw_transactions.size() != resp.added_transactions.size())
		throw std::runtime_error("qcoind error - SyncMemPool inconsistent transaction counts");
...
bool WalletState::redo_transaction(const PreparedWalletTransaction &pwtx, const std::vector<size_t> &global_indices,
	...
	// We may not generally know if transaction is ours or not if qcoind is lagged
