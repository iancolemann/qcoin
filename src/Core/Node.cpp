...
Node::Node(logging::ILogger &log, const Config &config, BlockChainState &block_chain)
	...
	if (!config.qcoind_bind_ip.empty() && config.qcoind_bind_port != 0)
		m_api = std::make_unique<http::Server>(config.qcoind_bind_ip, config.qcoind_bind_port,
...
bool Node::on_idle() {
	...
	if (m_block_chain.get_tip_height() >= m_block_chain.internal_import_known_height()) {
		for (size_t s = 0; s != 10; ++s) {
			...
			std::vector<P2PProtocolqCoin *> bp_copy{m_broadcast_protocols.begin(), m_broadcast_protocols.end()};
...
bool Node::on_api_http_request(http::Client *who, http::RequestBody &&request, http::ResponseBody &response) {
	...
	bool good_auth =
	    m_config.qcoind_authorization.empty() || request.r.basic_authorization == m_config.qcoind_authorization;
	bool good_auth_private = m_config.qcoind_authorization_private.empty() ||
	                         request.r.basic_authorization == m_config.qcoind_authorization_private;
...
void Node::broadcast(P2PProtocolqCoin *exclude, const BinaryArray &data) {
	for (auto &&p : m_broadcast_protocols)
		if (p != exclude)
			p->P2PProtocol::send(BinaryArray(data));  // Move is impossible here
}
void Node::broadcast(P2PProtocolqCoin *exclude, const BinaryArray &data_v1, const BinaryArray &data_v4) {
	for (auto &&p : m_broadcast_protocols)
		if (p != exclude)
...
bool Node::on_get_statistics(http::Client *, http::RequestBody &&http_request, json_rpc::Request &&,
	...
	bool good_auth_private = m_config.qcoind_authorization_private.empty() ||
	                         http_request.r.basic_authorization == m_config.qcoind_authorization_private;
