...
#include "trezor/messages-qcoin.hpp"
...
Trezor::Trezor(const std::string &path) : m_path(path), m_socket(platform::EventLoop::current()->io()) {
	...
	messages::qcoin::qCoinStartRequest req;

	//	messages::qcoin::qCoinStartRequest req2;
	//	messages::qcoin::qCoinStartRequest req3;
	//	auto tmp = req.SerializeAsString();
	//	protobuf::read(req2, tmp.begin(), tmp.end());
	//	req3.ParseFromString(protobuf::write(req2));

	auto http_resp =
	    trezor_post(m_socket, "/call/" + m_session, encode(req, messages::MessageType_qCoinStartRequest));
	messages::qcoin::qCoinStartResponse resp;
	if (!decode_any(resp, messages::MessageType_qCoinStartResponse, m_socket, m_session, http_resp.body))
		throw Exception("Trezor qcoin init message failed to decode, msg=" + http_resp.body);
	std::string major;
	std::string minor;
	...
	//	messages::qcoin::qCoinStartResponse resp2;
	//	messages::qcoin::qCoinStartResponse resp3;
	...
	//    common::uint_be_to_bytes(header, 2, unsigned(messages::MessageType_qCoinStartRequest));
	//    common::uint_be_to_bytes(header + 2, 4, str.size());
...
std::vector<cn::PublicKey> Trezor::scan_outputs(const std::vector<cn::PublicKey> &output_public_keys) {
	...
	acquire();
	messages::qcoin::qCoinScanOutputsRequest req;
	for (const auto &src : output_public_keys)
		req.output_public_key.push_back(common::as_string(src.data, sizeof(PublicKey)));
	auto http_resp =
	    trezor_post(m_socket, "/call/" + m_session, encode(req, messages::MessageType_qCoinScanOutputsRequest));
	messages::qcoin::qCoinScanOutputsResponse resp;
	if (!decode_any(resp, messages::MessageType_qCoinScanOutputsResponse, m_socket, m_session, http_resp.body))
		throw Exception("Trezor qcoin scan_outputs message failed to decode, msg=" + http_resp.body);
...
cn::KeyImage Trezor::generate_keyimage(const common::BinaryArray &output_secret_hash_arg, size_t address_index) {
	acquire();
	messages::qcoin::qCoinGenerateKeyimageRequest req;
	req.output_secret_hash_arg = common::as_string(output_secret_hash_arg);
	req.address_index          = static_cast<uint32_t>(address_index);
	auto http_resp =
	    trezor_post(m_socket, "/call/" + m_session, encode(req, messages::MessageType_qCoinGenerateKeyimageRequest));
	messages::qcoin::qCoinGenerateKeyimageResponse resp;
	if (!decode_any(resp, messages::MessageType_qCoinGenerateKeyimageResponse, m_socket, m_session, http_resp.body))
		throw Exception("Trezor qcoin generate_keyimage message failed to decode, msg=" + http_resp.body);
	KeyImage result;
...
Hash Trezor::generate_output_seed(const Hash &tx_inputs_hash, size_t out_index) {
	acquire();
	messages::qcoin::qCoinGenerateOutputSeedRequest req;
	...
	auto http_resp     = trezor_post(
        m_socket, "/call/" + m_session, encode(req, messages::MessageType_qCoinGenerateOutputSeedRequest));
	messages::qcoin::qCoinGenerateOutputSeedResponse resp;
	if (!decode_any(
	        resp, messages::MessageType_qCoinGenerateOutputSeedResponse, m_socket, m_session, http_resp.body))
		throw Exception("Trezor qcoin generate_output_seed message failed to decode, msg=" + http_resp.body);
	Hash result;
...
void Trezor::sign_start(size_t version, uint64_t ut, size_t inputs_size, size_t outputs_size, size_t extra_size) {
	acquire();
	messages::qcoin::qCoinSignStartRequest req;
	...
	auto http_resp =
	    trezor_post(m_socket, "/call/" + m_session, encode(req, messages::MessageType_qCoinSignStartRequest));
	messages::qcoin::qCoinEmptyResponse resp;
	if (!decode_any(resp, messages::MessageType_qCoinEmptyResponse, m_socket, m_session, http_resp.body))
		throw Exception("Trezor qcoin sign_start message failed to decode, msg=" + http_resp.body);
	release();
}
...
void Trezor::sign_add_input(uint64_t amount, const std::vector<size_t> &output_indexes,
	...
	acquire();
	messages::qcoin::qCoinSignAddInputRequest req;
	req.amount = amount;
	...
	req.address_index          = static_cast<uint32_t>(address_index);
	auto http_resp =
	    trezor_post(m_socket, "/call/" + m_session, encode(req, messages::MessageType_qCoinSignAddInputRequest));
	messages::qcoin::qCoinEmptyResponse resp;
	if (!decode_any(resp, messages::MessageType_qCoinEmptyResponse, m_socket, m_session, http_resp.body))
		throw Exception("Trezor qcoin sign_add_input message failed to decode, msg=" + http_resp.body);
	release();
}
...
void Trezor::sign_add_output(bool change, uint64_t amount, size_t change_address_index, uint8_t dst_address_tag,
	...
	acquire();
	messages::qcoin::qCoinSignAddOutputRequest req;
	req.change               = change;
	req.amount               = amount;
	...
	auto http_resp =
	    trezor_post(m_socket, "/call/" + m_session, encode(req, messages::MessageType_qCoinSignAddOutputRequest));
	messages::qcoin::qCoinSignAddOutputResponse resp;
	if (!decode_any(resp, messages::MessageType_qCoinSignAddOutputResponse, m_socket, m_session, http_resp.body))
		throw Exception("Trezor qcoin sign_add_output message failed to decode, msg=" + http_resp.body);
...
void Trezor::sign_add_extra(const BinaryArray &chunk) {
	...
	while (true) {
		...
		messages::qcoin::qCoinSignAddExtraRequest req;
		req.extra_chunk = common::as_string(chunk.data() + pos, stop - pos);
		auto http_resp =
		    trezor_post(m_socket, "/call/" + m_session, encode(req, messages::MessageType_qCoinSignAddExtraRequest));
		messages::qcoin::qCoinEmptyResponse resp;
		if (!decode_any(resp, messages::MessageType_qCoinEmptyResponse, m_socket, m_session, http_resp.body))
			throw Exception("Trezor qcoin sign_add_extra message failed to decode, msg=" + http_resp.body);
		pos = stop;
		if (pos == chunk.size())
			break;
	}
...
void Trezor::sign_step_a(const common::BinaryArray &output_secret_hash_arg, size_t address_index,
	...
	acquire();
	messages::qcoin::qCoinSignStepARequest req;
	req.output_secret_hash_arg = common::as_string(output_secret_hash_arg);
	req.address_index          = static_cast<uint32_t>(address_index);
	auto http_resp =
	    trezor_post(m_socket, "/call/" + m_session, encode(req, messages::MessageType_qCoinSignStepARequest));
	messages::qcoin::qCoinSignStepAResponse resp;
	if (!decode_any(resp, messages::MessageType_qCoinSignStepAResponse, m_socket, m_session, http_resp.body))
		throw Exception("Trezor qcoin sign_step_a message failed to decode, msg=" + http_resp.body);
...
void Trezor::sign_step_a_more_data(const BinaryArray &data) {
	...
	while (true) {
		...
		messages::qcoin::qCoinSignStepAMoreDataRequest req;
		req.data_chunk = common::as_string(data.data() + pos, stop - pos);
		auto http_resp = trezor_post(
		    m_socket, "/call/" + m_session, encode(req, messages::MessageType_qCoinSignStepAMoreDataRequest));
		messages::qcoin::qCoinEmptyResponse resp;
		if (!decode_any(resp, messages::MessageType_qCoinEmptyResponse, m_socket, m_session, http_resp.body))
			throw Exception("Trezor qcoin sign_step_a_more_data message failed to decode, msg=" + http_resp.body);
		pos = stop;
		if (pos == data.size())
			break;
	}
...
crypto::EllipticCurveScalar Trezor::sign_get_c0() {
	acquire();
	messages::qcoin::qCoinSignGetC0Request req;
	auto http_resp =
	    trezor_post(m_socket, "/call/" + m_session, encode(req, messages::MessageType_qCoinSignGetC0Request));
	messages::qcoin::qCoinSignGetC0Response resp;
	if (!decode_any(resp, messages::MessageType_qCoinSignGetC0Response, m_socket, m_session, http_resp.body))
		throw Exception("Trezor qcoin sign_get_c0 message failed to decode, msg=" + http_resp.body);
...
void Trezor::sign_step_b(const common::BinaryArray &output_secret_hash_arg, size_t address_index,
	...
	acquire();
	messages::qcoin::qCoinSignStepBRequest req;
	req.output_secret_hash_arg = common::as_string(output_secret_hash_arg);
	req.address_index          = static_cast<uint32_t>(address_index);
	req.my_c                   = common::as_string(my_c.data, sizeof(crypto::EllipticCurveScalar));
	auto http_resp =
	    trezor_post(m_socket, "/call/" + m_session, encode(req, messages::MessageType_qCoinSignStepBRequest));
	messages::qcoin::qCoinSignStepBResponse resp;
	if (!decode_any(resp, messages::MessageType_qCoinSignStepBResponse, m_socket, m_session, http_resp.body))
		throw Exception("Trezor qcoin sign_step_b message failed to decode, msg=" + http_resp.body);
...
void Trezor::proof_start(const common::BinaryArray &data) {
	acquire();
	messages::qcoin::qCoinStartProofRequest req;
	req.data_size = common::integer_cast<uint32_t>(data.size());
	auto http_resp =
	    trezor_post(m_socket, "/call/" + m_session, encode(req, messages::MessageType_qCoinStartProofRequest));
	messages::qcoin::qCoinEmptyResponse resp;
	if (!decode_any(resp, messages::MessageType_qCoinEmptyResponse, m_socket, m_session, http_resp.body))
		throw Exception("Trezor qcoin proof_start message failed to decode, msg=" + http_resp.body);
	release();
	sign_add_extra(data);
}
...
void Trezor::export_view_only(SecretKey *audit_key_base_secret_key, SecretKey *view_secret_key, Hash *view_seed,
	...
	acquire();
	messages::qcoin::qCoinExportViewWalletRequest req;
	auto http_resp =
	    trezor_post(m_socket, "/call/" + m_session, encode(req, messages::MessageType_qCoinExportViewWalletRequest));
	messages::qcoin::qCoinExportViewWalletResponse resp;
	if (!decode_any(resp, messages::MessageType_qCoinExportViewWalletResponse, m_socket, m_session, http_resp.body))
		throw Exception("Trezor qcoin export_view_only message failed to decode, msg=" + http_resp.body);
