...
Options:
  ...
  --qcoind-address=<ip:port>  Single option for both daemon address and port.
...
struct MiningConfig {
	explicit MiningConfig(common::CommandLine &cmd)
	    : qcoind_ip("127.0.0.1"), qcoind_port(parameters::RPC_DEFAULT_PORT) {
		...
		if (const char *pa = cmd.get("--" CRYPTONOTE_NAME "d-address")) {
			ewrap(common::parse_ip_address_and_port(pa, &qcoind_ip, &qcoind_port),
	...
	std::string qcoind_ip;
	std::string boast;
	uint16_t qcoind_port = 0;
...
public:
	...
	explicit HTTPMiner(const MiningConfig &mining_config)
	    ...
	    , getwork_agent(mining_config.qcoind_ip, mining_config.qcoind_port)
	    , submit_agent(mining_config.qcoind_ip, mining_config.qcoind_port)
