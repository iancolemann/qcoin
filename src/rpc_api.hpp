...
// qcoin does not use fractional numbers, but uses numbers as large as 2^64-1 for amounts,
...
// qcoin does not use Null, you should specify empty Array as [], empty string as "", empty Object as {}
...
struct RawBlock {
	...
	std::vector<api::Transaction>
	    transactions;  // for each transaction + coinbase, contain only info known to qcoind
...
struct GetStatus {
	...
	struct Response {  // Response and Request have fields in common.
		...
		// Problems on lower levels (like qcoind errors in walletd status). Empty - no errors
		Height top_block_height                              = 0;
...
// These messages encoded in JSON can be sent via http url /json_rpc3 to qcoind rpc address:port
// or to binMethod() url encoded in unspecified binary format
namespace cn { namespace api { namespace cnd {  // cryptonoted is historically compiled to qcoind, etc.
...
struct SyncBlocks {  // Used by walletd, block explorer, etc to sync to qcoind
...
struct GetRawTransaction {
	...
	struct Response {
		api::Transaction transaction;  // contain only info known to qcoind
...
struct SyncMemPool {  // Used by walletd sync process
	...
	struct Response {
		...
		std::vector<api::Transaction> added_transactions;       // contain only info known to qcoind
		GetStatus::Response status;  // We save roundtrip during sync by also sending status here
	};
};
...
void ser_members(cn::api::Output &v, ISeria &s, bool only_qcoind_fields = false);
