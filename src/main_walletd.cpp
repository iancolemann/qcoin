...
static const char USAGE[] = R"(walletd )" qcoin_VERSION_STRING R"(.
...
Running with selected wallet:
  ...
  --data-folder=<folder-path>           Folder for wallet cache, blockchain, logs and peer DB [default: %appdata%/qcoin].
  --qcoind-remote-address=<ip:port>  Connect to remote qcoind and suppress running built-in qcoind.
                                        Set this option to https://<host:port> instead, to connect to remote qcoind via https
  --qcoind-authorization=<user:pass> HTTP basic authentication credentials for RPC API.
...
Options for built-in qcoind (run when no --qcoind-remote-address specified):
DEPRECATED AND NOT RECOMMENDED as entailing security risk. Please always run qcoind as a separate process.
  ...
  --qcoind-bind-address=<ip:port>    IP and port for qcoind RPC [default: 127.0.0.1:8081].
...
static const bool separate_thread_for_qcoind = true;
...
//  --qcoind-remote-address=<ip:port>  Connect to remote qcoind...
//  --qcoind-authorization=<user:pass> HTTP basic authentication credentials for RPC API.
...
int main(int argc, const char *argv[]) try {
	...
	try {
		...
		std::cout << "qCoind already running - " << common::what(ex) << std::endl;
	...
	// Carefull, throwing after we create qcoind thread will terminate immediately
	std::promise<void> prm;
	std::thread qcoind_thread;
	if (!config.qcoind_remote_port) {
		try {
			if (separate_thread_for_qcoind) {
				qcoind_thread      = std::thread([&prm, &logManagerNode, &config, &currency] {
                    boost::asio::io_service io;
			...
			if (qcoind_thread.joinable())
				qcoind_thread.join();  // otherwise terminate will be called in ~thread
