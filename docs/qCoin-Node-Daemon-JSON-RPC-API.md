...
By default, qCoin Node Service (`qcoind`, Node Daemon) is bound only to `127.0.0.1` (`localhost`) interface, so it can be accessed only from the same computer it runs on. This is done to reduce number of external attack vectors. `qcoind` itself has access to only public information, but it sometimes runs in the same process with `walletd`, which has access to wallet keys. To bind `qcoind` to all network interfaces, use `--qcoind-bind-address=0.0.0.0:8081` command line argument (specifying port is mandatory).

To make a JSON PRC request to the `qcoind` you should make an HTTP POST request to an entry point:
```
http://<ip>:<port>/json_rpc
```
where:
* `<ip>` is IPv4 address of `qcoind` service. If the service is on local machine, use `127.0.0.1` instead of `localhost`.
* `<port>` is TCP port of `qcoind`. By default the service is bound to `8081`.
...
| 5.  | `get_statistics`      | Gets statistics about running `qcoind`.                    |
| 6.  | `get_status`          | Returns status of `qcoind`.                                |
...
| Field              | Type             | Description                                                           |
|--------------------|------------------|-----------------------------------------------------------------------|
| `transaction`      | `Transaction`    | Standard Transaction object. Contains info only known to `qcoind`. |
...
Returns misc statistics about `qcoind` being queried.
...
| `start_time`                           | `timestamp`    | Timestamp of `qcoind` start time in UTC.              |
...
| `peer_list_white`                      | `[]Peer`       | Peers `qcoind` has successfully connected to.         |
...
Let's make a `get_statistics` query to `qcoind`.
...
Get status about state of `qcoind`. This method supports longpolling. If you specify all input parameters, \
and they are equal to the current state of the `qcoind`, you will get response only when some of them change. \
...
| Field                              | Type     | Description                                                                              |
|------------------------------------|----------|------------------------------------------------------------------------------------------|
| `incoming_peer_count`              | `uint32` | Incoming peers to qcoind.                                                             |
| `lower_level_error`                | `string` | Error on lower level (qcoind for walletd, etc).                                       |
...
| `outgoing_peer_count`              | `uint32` | Outgoing peers from qcoind.                                                           |
...
| `top_block_height`                 | `uint32` | All transaction prior to that height have been processed by qcoind.                   |
...
| `first_block_timestamp`    | `timestamp` | No        | `0`           | `qcoind` won't return blocks earlier than this point in time. |
...
Result of call will be `broadcast`, if transaction was successfully placed into the payment queue. Note, that if `qcoind` is not connected to internet, this method will nevertheless succeed.
...
Let's send a previously created transaction by submitting `binary_transaction` to `qcoind`.
